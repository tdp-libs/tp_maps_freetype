  ...
  {
    {
      ...
      if ( error )
      {
        tpDebug() << "Failed to initialize free type: " << ft_errors[error].err_msg;
      }
    }

    {
      ...
      {
        tpDebug() << "New face error: " << ft_errors[error].err_msg;
      }
    }

    {
      ...
      if ( error )
            {
              tpDebug() << "Set char size error: " << ft_errors[error].err_msg;
            }
    }
  }
...
#warning free stuff
  ...
  void FreeTypeFont::prepareGlyph(char16_t character, const std::function<void(const tp_maps::Glyph&)>& addGlyph) const
  {
    ...
    {
      tpDebug() << "Failed to load glyph: " << ft_errors[error].err_msg;
      return;
    }
    ...
    if(d->face->glyph->format != FT_GLYPH_FORMAT_BITMAP)
    {
      ...
      {
        tpDebug() << "Failed to render glyph: " << ft_errors[error].err_msg;
        return;
      }
    }
